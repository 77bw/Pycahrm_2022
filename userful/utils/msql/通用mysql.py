...
class MysqlClient(object):
    ...
    def get_jsonData(self, data):
        ...
        keys = ['`{}`'.format(key) for key in data[0].keys()]
        keys = self.list2str(keys).replace("'", '')
        values = [tuple(data[i].values()) for i in range(0, len(data))]
        return keys, values
...
if __name__ == '__main__':
    ...
    # sql_conn.insert_one('demo', data)
    # values = [sql_conn.format_sql_value(value) for i in data for value in data.values() ]
    # values = sql_conn.list2str(values)
    # print(values)